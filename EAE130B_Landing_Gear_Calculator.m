%% Landing Gear Calculator (in imperial units) 
...
Load_check_M_f = M_f/B;
    ...
    if Load_check_M_a > 0.05 
         fprintf('M_a/B ratio "%.2f" is sufficient, it is greater than 0.05\n', Load_check_M_a)
    else 
         fprintf('M_a/B ratio "%.2f" is not sufficient, decrease B value and/or increase M_a value to increase the ratio to above 0.05\n', Load_check_M_a)
    end
   
    if Load_check_M_f < 0.20
         fprintf('M_f/B ratio "%.2f" is sufficent, it is less than 0.20\n', Load_check_M_f)
    else 
         fprintf('M_f/B ratio "%.2f" is not sufficent, increase B value and/or decrease M_f to decrease the ratio to below 0.20\n', Load_check_M_f)
    end 

% Tire Sizing Table 
Parameter = ["Total weight allowed"; "Kinetic energy per braked wheel"; "Main tire diameter"; "Nose tire diameter"; "Main tire radius"; "Nose tire radius"; "Main tire width"; "Nose tire width"; "Main tire pressure"; "Nose tire pressure"];
Value = [W_allowed; KE_braking; Main_tire_diameter; Nose_tire_diameter; Main_tire_radius; Nose_tire_radius; Main_tire_width; Nose_tire_width; Main_tire_pressure; Nose_tire_pressure];
Units = ["lbf"; "ftlb/s"; "in"; "in"; "in"; "in"; "in"; "in"; "psi"; "psi"];
Table = table(Parameter, Value, Units)
