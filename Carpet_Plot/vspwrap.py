...
def runVSP(W_dg: float = 42449, M_cruise: float = 0.85, M_dash: float = 1.6, get_CD0: bool = False, get_CD_wave: bool = False,
           geom_def: str | dict = r'C:\Users\14153\Desktop\Skewl\EAE 130\Python\EAE130_aircraft_design\Flying_Surfaces\airplane_geom2.json',
    ...
    # 
    base_fuse = r"C:\Users\14153\Desktop\Skewl\EAE 130\Python\EAE130_aircraft_design\VSP_Files\5_SIMPLE_F24HH_FUSE.vsp3"
    base_wing_foils = [r"C:\Users\14153\Desktop\Airfoil Library\NACA 64A006_TEST.dat", r"C:\Users\14153\Desktop\Airfoil Library\NACA 64A005.dat", r"C:\Users\14153\Desktop\Airfoil Library\NACA 64A004_TEST.dat"]
    base_tail_foils = [r"C:\Users\14153\Desktop\Airfoil Library\NACA 65A004.dat", r"C:\Users\14153\Desktop\Airfoil Library\NACA 65A005.dat"]

    # --- 2. COPY TO ISOLATED WORKER DIRECTORY ---
    # Copy Fuselage
    local_fuse = os.path.join(save_dir, "local_fuse.vsp3")
    shutil.copy(base_fuse, local_fuse)

    # Copy Wing Foils
    local_wing_foils = []
    for i, foil in enumerate(base_wing_foils):
        local_f = os.path.join(save_dir, f"wing_foil_{i}.dat")
        shutil.copy(foil, local_f)
        local_wing_foils.append(local_f)

    # Copy Tail Foils
    local_tail_foils = []
    for i, foil in enumerate(base_tail_foils):
        local_f = os.path.join(save_dir, f"tail_foil_{i}.dat")
        shutil.copy(foil, local_f)
        local_tail_foils.append(local_f)

    # Configure VSPInterface
    config = vspa.Config(vsp_filename=fname,
                        W_dg=W_dg,
                        geom_def_path=geom_def,
                        fuse_file_path=local_fuse,
                        wing_foils=local_wing_foils,
                        tail_foils=local_tail_foils)
