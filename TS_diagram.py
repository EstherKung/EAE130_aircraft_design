...
chosen_point = {'s': 430, 't':37000}
plt.scatter(500, 44000, marker='*', color='navy', label='FA-18 E/F')
plt.scatter(460, 41000, marker='*', color='goldenrod', label='F-35C')
plt.scatter(chosen_point['s'], chosen_point['t'], marker='*', color='gray', label='chosen point')
W_chosen, _, _ = weight_convergence(50000, T_0 = chosen_point['t'], 
                                    S={'wing': chosen_point['s'], 'htail': 78.41, 'vtail': 78.41, 'fuse_wet': 529.81}, 
                                    mission='strike')
plt.annotate(f"MTOW: {W_chosen:.0f} lbs \n (strike mission)", (chosen_point['s']+10, chosen_point['t']-2000), fontsize=10, color='gray')
